...
class FavoritesView(Canvas):
    def __init__(self, parent, controller: Window):
        ...
        _text = "This is an example of a word element, you can either create a label or a button with this"
        self.controller.settingsUnpacker(self.staticImgs, "label")
        # Export the image from Figma as a png, and place it in the Assets folder under 
        # the respective folder, and then use the relative path to the image
        # by right clicking at the bar on the left of the screen
        im = r"Assets\FavoritesView\whitebackdroplavenderfront.png"
        self.controller.textElement(
            # This im background is 1720 x 100, as made from Figma
            # and placed according to the grid
            imagepath=im, xpos=100, ypos=440,
    ...
    # You will have to wait a moment for your client to connect to the DB
    # After that, this method will be ran without blocking the main thread

    def postLogin(self, prisma: Prisma = None, data: dict = None):
        self.userId = data["id"]
        self.fullName = data["fullName"]
        self.email = data["email"]
        modules = data["modules"]
        # alias for self.controller for shorter code
        c = self.controller
        im = r"Assets\FavoritesView\whitebackdroplavenderfront.png"
        nameText = f"List of Favorite Posts for {self.fullName}"
        initCoords = (100, 120)
        c.textElement(
            imagepath=im, xpos=initCoords[0], ypos=initCoords[1],
            classname=f"title", root=self,
            text=nameText, size=34, font=INTERBOLD,
        )
        # Your task is to design and create buttons and labels for Favorite Posts 
        # returned from the database queries using Prisma.
        user = prisma.userprofile.find_first(
            where={
                "id": self.userId
            },
        ...
        # user.favoritePosts is a list of ModulePost objects
        # You can access the fields like this
        for post in user.favoritePosts:
            print(f"Post title:{post.title}")
            print(f"Post content:{post.author.fullName}")
        # Use a suitable data structure to collect the information you want to display
        # and then use a for loop to create the buttons and labels
        # You can use the exampleofawordelement as a template but
        # experimenting in Figma should be done to save some time   
